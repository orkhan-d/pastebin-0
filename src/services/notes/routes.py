...
async def create_note(data: schemas.CreateNoteSchema,
    ...
    local_filename = f'{uuid4()}.txt'

    try:
        # save file locally
        async with aiofiles.open(local_filename, mode='w') as f:
            await f.write(data.content)

        # upload file to S3
        await S3().upload_file(local_filename)

        # add note to database
        note = await create_note_db(data, db_session)
